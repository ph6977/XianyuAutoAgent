...
class BatchUserCrawler:
        ...
        try:
            ...
            for attempt in range(max_scroll_attempts):
                ...
                for selector in user_name_selectors:
                    user_elements = chat_list.eles(selector)
                    for element in user_elements:
                        text = element.text.strip()
                        if text and len(text) > 1 and len(text) < 30:
                            # 排除系统文本
                            if not any(keyword in text.lower() for keyword in ['搜索', '联系人', '消息', '聊天', '最近', '全部', '暂无', '通知', '通', '知']):
                                if text not in user_nicknames:
                                    user_nicknames.append(text)
                                    logger.info(f"找到用户: {text} (选择器: {selector})")

                # 方法2: 备用方法 - 获取所有文本元素进行过滤
                if len(user_nicknames) < max_users:
                    all_text_elements = chat_list.eles('text=.*')
                    ...
                    for element in all_text_elements:
                        ...
                        if text and len(text) > 1 and len(text) < 200:
                            ...
                            if not any(keyword in text.lower() for keyword in ['搜索', '联系人', '消息', '聊天', '最近', '全部', '暂无', '通知', '通', '知']):
                                # 尝试从组合文本中提取用户昵称
                                extracted_nickname = self._extract_nickname_from_text(text)
                                if extracted_nickname and extracted_nickname not in user_nicknames:
                                    user_nicknames.append(extracted_nickname)
                                    logger.info(f"找到用户: {extracted_nickname} (原始文本: {text})")
    ...
    def _click_user_in_chat_list(self, nickname: str) -> bool:
        ...
        try:
            ...
            for selector in user_name_selectors:
                elements = chat_list.eles(selector)
                for element in elements:
                    if nickname in element.text:
                        # 找到包含用户昵称的元素，点击其父级用户按钮
                        parent_button = element.parent
                        if parent_button:
                            user_elements.append(parent_button)
                            logger.info(f"找到用户按钮: {nickname} (选择器: {selector})")
