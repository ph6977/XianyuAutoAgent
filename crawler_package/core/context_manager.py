"""
对话上下文管理器 - 管理聊天历史和商品信息
"""

import json
import os
import time
from typing import List, Dict, Optional
from loguru import logger


class ChatContextManager:
    """聊天上下文管理器"""

    def __init__(self, db_path: str = "chat_history.json"):
        self.db_path = db_path
        self._ensure_db_exists()

    def _ensure_db_exists(self):
        """确保数据库文件存在"""
        if not os.path.exists(self.db_path):
            with open(self.db_path, 'w', encoding='utf-8') as f:
                json.dump({
                    "chats": {},
                    "items": {},
                    "metadata": {
                        "created_at": time.time(),
                        "last_updated": time.time()
                    }
                }, f, ensure_ascii=False, indent=2)

    def _load_db(self) -> Dict:
        """加载数据库"""
        try:
            with open(self.db_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            logger.error(f"加载数据库失败: {e}")
            return {"chats": {}, "items": {}, "metadata": {"created_at": time.time(), "last_updated": time.time()}}

    def _save_db(self, data: Dict):
        """保存数据库"""
        try:
            data["metadata"]["last_updated"] = time.time()
            with open(self.db_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception as e:
            logger.error(f"保存数据库失败: {e}")

    def add_message_by_chat(self, chat_id: str, user_id: str, item_id: str, role: str, content: str, message_type: str = "text"):
        """添加消息到聊天记录"""
        data = self._load_db()

        if chat_id not in data["chats"]:
            data["chats"][chat_id] = {
                "chat_id": chat_id,
                "item_id": item_id,
                "user_id": user_id,
                "messages": [],
                "bargain_count": 0,
                "original_price": None,  # 标价
                "final_price": None,    # 成交价
                "discount_amount": None, # 折扣金额
                "discount_percentage": None, # 折扣百分比
                "shipping_info": None,  # 邮费信息
                "is_free_shipping": False, # 是否包邮
                "deal_status": "pending", # 交易状态: pending, completed, cancelled
                "bargain_history": [],  # 议价过程
                "first_message_content": content if role == "user" else None, # 首次消息内容
                "response_time_avg": None, # 平均响应时间
                "total_session_time": None, # 会话总时长
                "return_status": None,  # 退货状态
                "product_condition": None, # 商品成色
                "posting_time": None,  # 发布时间
                "negotiation_pattern": None, # 议价模式
                "seller_response_pattern": None, # 卖家回应模式
                "created_at": time.time(),
                "last_updated": time.time()
            }

        message = {
            "role": role,
            "content": content,
            "timestamp": time.time(),
            "message_type": message_type, # 消息类型: text, price_negotiation, order_status等
            "is_bargain_related": self._is_bargain_message(content) # 是否与议价相关
        }

        data["chats"][chat_id]["messages"].append(message)
        data["chats"][chat_id]["last_updated"] = time.time()

        self._save_db(data)

    def _is_bargain_message(self, content: str) -> bool:
        """判断消息是否与议价相关"""
        bargain_keywords = [
            "便宜", "优惠", "还价", "砍价", "打折", "降价", "价格", "多少钱", "贵", "便宜点",
            "多少", "价位", "底价", "让利", "成交价", "原价", "折扣", "促销", "特价", 
            "1块", "一块", "少点", "给个价", "能少", "价格能", "可以便宜", "能便宜"
        ]
        return any(keyword in content for keyword in bargain_keywords)

    def get_context_by_chat(self, chat_id: str) -> List[Dict]:
        """获取聊天上下文"""
        data = self._load_db()

        if chat_id not in data["chats"]:
            return []

        return data["chats"][chat_id]["messages"]

    def save_item_info(self, item_id: str, item_info: Dict):
        """保存商品信息"""
        data = self._load_db()

        data["items"][item_id] = {
            "item_id": item_id,
            "info": item_info,
            "saved_at": time.time()
        }

        self._save_db(data)

    def get_item_info(self, item_id: str) -> Optional[Dict]:
        """获取商品信息"""
        data = self._load_db()

        if item_id in data["items"]:
            return data["items"][item_id]["info"]

        return None

    def increment_bargain_count_by_chat(self, chat_id: str):
        """增加议价次数"""
        data = self._load_db()

        if chat_id in data["chats"]:
            data["chats"][chat_id]["bargain_count"] += 1
            self._save_db(data)

    def get_bargain_count_by_chat(self, chat_id: str) -> int:
        """获取议价次数"""
        data = self._load_db()

        if chat_id in data["chats"]:
            return data["chats"][chat_id]["bargain_count"]

        return 0

    def update_deal_info_by_chat(self, chat_id: str, original_price: float = None, 
                                 final_price: float = None, shipping_info: str = None,
                                 is_free_shipping: bool = None, deal_status: str = None,
                                 product_condition: str = None, posting_time: str = None):
        """更新交易信息"""
        data = self._load_db()

        if chat_id in data["chats"]:
            if original_price is not None:
                data["chats"][chat_id]["original_price"] = original_price
            if final_price is not None:
                data["chats"][chat_id]["final_price"] = final_price
            if shipping_info is not None:
                data["chats"][chat_id]["shipping_info"] = shipping_info
            if is_free_shipping is not None:
                data["chats"][chat_id]["is_free_shipping"] = is_free_shipping
            if deal_status is not None:
                data["chats"][chat_id]["deal_status"] = deal_status
            if product_condition is not None:
                data["chats"][chat_id]["product_condition"] = product_condition
            if posting_time is not None:
                data["chats"][chat_id]["posting_time"] = posting_time
            
            # 计算折扣信息
            if original_price is not None and final_price is not None:
                data["chats"][chat_id]["discount_amount"] = original_price - final_price
                data["chats"][chat_id]["discount_percentage"] = ((original_price - final_price) / original_price) * 100 if original_price > 0 else 0
            
            data["chats"][chat_id]["last_updated"] = time.time()
            self._save_db(data)

    def add_bargain_event(self, chat_id: str, original_price: float, counter_price: float, 
                          message_content: str, timestamp: float = None):
        """添加议价事件"""
        data = self._load_db()

        if chat_id in data["chats"]:
            if timestamp is None:
                timestamp = time.time()
            
            bargain_event = {
                "timestamp": timestamp,
                "original_price": original_price,
                "counter_price": counter_price,
                "message_content": message_content,
                "price_difference": abs(original_price - counter_price)
            }
            
            data["chats"][chat_id]["bargain_history"].append(bargain_event)
            data["chats"][chat_id]["bargain_count"] += 1
            data["chats"][chat_id]["last_updated"] = time.time()
            
            self._save_db(data)

    def get_deal_statistics(self) -> Dict:
        """获取交易统计信息"""
        data = self._load_db()
        chats = data["chats"]

        total_chats = len(chats)
        completed_deals = 0
        total_discount_amount = 0
        total_original_price = 0
        free_shipping_count = 0
        bargain_chats_count = 0

        for chat in chats.values():
            if chat.get("deal_status") == "completed":
                completed_deals += 1
                if chat.get("original_price") and chat.get("final_price"):
                    total_original_price += chat["original_price"]
                    total_discount_amount += (chat["original_price"] - chat["final_price"])
            
            if chat.get("is_free_shipping"):
                free_shipping_count += 1
                
            if chat.get("bargain_count", 0) > 0:
                bargain_chats_count += 1

        avg_discount_per_deal = (total_discount_amount / completed_deals) if completed_deals > 0 else 0
        avg_discount_percentage = ((total_discount_amount / total_original_price) * 100) if total_original_price > 0 else 0
        bargain_rate = (bargain_chats_count / total_chats) * 100 if total_chats > 0 else 0
        free_shipping_rate = (free_shipping_count / total_chats) * 100 if total_chats > 0 else 0

        return {
            "total_chats": total_chats,
            "completed_deals": completed_deals,
            "bargain_chats_count": bargain_chats_count,
            "bargain_rate": bargain_rate,
            "free_shipping_count": free_shipping_count,
            "free_shipping_rate": free_shipping_rate,
            "total_discount_amount": total_discount_amount,
            "avg_discount_per_deal": avg_discount_per_deal,
            "avg_discount_percentage": avg_discount_percentage,
            "completed_deal_rate": (completed_deals / total_chats) * 100 if total_chats > 0 else 0
        }

    def get_all_chats(self) -> List[Dict]:
        """获取所有聊天记录"""
        data = self._load_db()
        return list(data["chats"].values())

    def get_chat_statistics(self) -> Dict:
        """获取聊天统计信息"""
        data = self._load_db()
        chats = data["chats"]

        total_chats = len(chats)
        total_messages = sum(len(chat["messages"]) for chat in chats.values())
        total_bargain_count = sum(chat["bargain_count"] for chat in chats.values())

        avg_messages_per_chat = total_messages / total_chats if total_chats > 0 else 0

        latest_chat_time = None
        if chats:
            # 计算最后更新时间，使用消息中的最大时间戳
            all_timestamps = []
            for chat in chats.values():
                if chat['messages']:
                    all_timestamps.extend([msg['timestamp'] for msg in chat['messages']])
            if all_timestamps:
                latest_chat_time = max(all_timestamps)

        return {
            "total_chats": total_chats,
            "total_messages": total_messages,
            "total_bargain_count": total_bargain_count,
            "avg_messages_per_chat": avg_messages_per_chat,
            "latest_chat_time": latest_chat_time
        }