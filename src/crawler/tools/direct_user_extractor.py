...
class DirectUserExtractor:
    ...
    def extract_user_nicknames_direct(self, max_users: int = 20) -> List[str]:
        """
        直接提取聊天列表中的用户昵称 - 基于HTML元素结构
        ...
        logger.info(f"开始直接提取聊天列表中的前 {max_users} 个用户昵称...")
        ...
        try:
            ...
            for attempt in range(max_scroll_attempts):
                ...
                # 直接查找包含用户昵称的HTML元素
                # 基于您提供的示例：<div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 180px;">爱喝咖啡奶茶的KK</div>
                chat_users = self.page.run_js("""
                    function getDirectUsers() {
                        const users = [];

                        // 方法1：查找包含用户昵称的特定div元素
                        const userDivs = document.querySelectorAll('div[style*="overflow: hidden"][style*="text-overflow: ellipsis"][style*="white-space: nowrap"]');

                        for (const div of userDivs) {
                            const text = div.textContent.trim();
                            if (text && text.length > 1 && text.length < 30) {
                                // 检查是否是有效的用户昵称
                                if (isValidDirectNickname(text)) {
                        ...
                        // 方法2：查找聊天列表中的用户元素
                        const chatList = document.querySelector('.rc-virtual-list-holder') ||
                                        document.querySelector('.conversation-list--jDBLEMex') ||
                                        document.querySelector('.conversation-item--JReyg97P');

                        if (chatList) {
                            // 在聊天列表中查找可能的用户昵称元素
                            const potentialUsers = chatList.querySelectorAll('div, span, a');
                            for (const element of potentialUsers) {
                    ...
                    function isValidDirectNickname(text) {
                        // 基础长度检查
                        ...
                        const excludeKeywords = [
                            '搜索', '联系人', '消息', '通知', '最近', '全部', '暂无',
                            '订单', '交易', '退款', '成功', '关闭', '评价', '快给',
                            '小时前', '分钟前', '天前', '刚刚', '昨天', '前天',
                            '车还在', '续航', '自提', '邮寄', '包邮', '最低', '券',
                            '优惠', '发闲置', '反馈', '客服', '回顶部', '登录'
                        ];
                        ...
                        const validPatterns = [
                            /^[\\u4e00-\\u9fa5]{2,15}$/,  // 2-15个中文字符
                            /^[\\u4e00-\\u9fa5]+[\\u4e00-\\u9fa5\\w]*$/,  // 中文开头，可包含字母数字
                            /^[a-zA-Z]+[\\u4e00-\\u9fa5\\w]*$/,  // 字母开头，可包含中文和数字
                            /^[\\u4e00-\\u9fa5]+的[\\u4e00-\\u9fa5]+$/,  // "某某的某某"格式
                            /^[\\u4e00-\\u9fa5]+小[\\u4e00-\\u9fa5]+$/,  // "小某某"格式
                            /^[a-zA-Z]\\*+[0-9]$/,  // "x***1"格式
                            /^[\\u4e00-\\u9fa5]+[\\u4e00-\\u9fa5\\w\s]*$/,  // 中文开头，可包含空格
                        ];
                        ...
                        if (text.length >= 2 && text.length <= 20 &&
                            ...
                            !text.includes('小时') && !text.includes('分钟') && !text.includes('天前')) {
                            return true;
                        }
                ...
                # 滚动聊天列表
                try:
                    chat_list = self.page.ele('.rc-virtual-list-holder') or self.page.ele('.conversation-list--jDBLEMex')
                    if chat_list and chat_list != self.page:
                        try:
                ...
                # 检查是否已经滚动到底部
                chat_list = self.page.ele('.rc-virtual-list-holder') or self.page.ele('.conversation-list--jDBLEMex')
                current_scroll = self.page.run_js("return arguments[0].scrollTop", chat_list) if chat_list and chat_list != self.page else self.page.run_js("return window.pageYOffset")
    ...
    def save_user_list(self, user_nicknames: List[str], output_file: str = "direct_chat_list_users.txt") -> bool:
        """
        保存用户列表到文件
...
def main():
    ...
    parser = argparse.ArgumentParser(description='直接用户昵称提取器 - 基于HTML元素结构')
    parser.add_argument('--max-users', type=int, default=0, help='最大用户数量（0表示交互式输入，默认：0）')
    parser.add_argument('--output-file', default='direct_chat_list_users.txt', help='输出文件路径（默认：direct_chat_list_users.txt）')
    ...
    try:
        # 创建用户提取器
        extractor = DirectUserExtractor(headless=args.headless)
        ...
        user_nicknames = extractor.extract_user_nicknames_direct(max_users=args.max_users)
