...
class OptimizedBatchUserCrawler:
    ...
    def _get_chat_list_users(self, max_users: int = 10) -> List[str]:
        """
        获取聊天列表中的用户昵称 - 改进的逻辑：先查找当前页面用户，再滚动
        ...
        try:
            ...
            scroll_height = 400  # 增加滚动高度
            previous_user_count = 0  # 记录之前的用户数量
            no_new_user_count = 0  # 记录连续没有新用户的次数

            # 用户昵称选择器 - 基于实际闲鱼页面结构
            nickname_selectors = [
                # 您提供的HTML元素样式 - 最可能的用户昵称元素
                ...
                '[class*="nickname"]',
                '[class*="contact-name"]',
                '[class*="conv"] div[style*="overflow: hidden"]',
                # 基础选择器
                'div[style*="font-weight: 500"]',
                'div[style*="color: rgb(51, 51, 51)"]',
                'div[style*="max-width"]',
                # 备用选择器
                '.conversation-item--JReyg97P div',
                '.conv-item div',
                '[class*="item"] div',
            ...
            for attempt in range(max_scroll_attempts):
                    ...
                    try:
                        ...
                        for element in elements:
                            ...
                            if text and len(text) > 1 and len(text) < 30:
                                ...
                                # 检查是否是数字或时间（聊天时间戳）
                                is_time = ':' in text and any(c.isdigit() for c in text) and len(text) <= 8
                                # 检查是否是金额（包含¥符号）
                                is_amount = '¥' in text or '元' in text
                                # 检查是否是数字ID
                                is_id = text.isdigit() and len(text) > 5
                                # 检查是否太短（如"我"、"好"等）
                                is_short = len(text) == 1 and text in '我你他她它好是的了在了有和就也还但又或等'
                                
                                if (not is_system and not is_time and not is_amount and 
                                    not is_id and not is_short and text not in user_nicknames):
                                    # 额外检查：可能是用户昵称的特征
                                    # 包含中文字符、英文字母、数字、下划线等
                                    import re
                                    chinese_char = re.search(r'[\u4e00-\u9fff]', text)
                                    english_char = re.search(r'[a-zA-Z]', text) 
                                    numeric_char = re.search(r'[0-9]', text)
                                    underscore = '_' in text
                                    dash = '-' in text
                                    
                                    # 用户昵称通常包含中文、英文、数字或特殊字符
                                    is_possible_nickname = (chinese_char or english_char or numeric_char or 
                                                          underscore or dash or len(text) > 1)
                                    
                                    if is_possible_nickname:
                                        user_nicknames.append(text)
                                        logger.info(f"找到用户: {text}")
                                        # 如果找到足够用户，停止查找
                                        if len(user_nicknames) >= max_users:
                                            logger.info(f"已找到 {len(user_nicknames)} 个用户，停止查找")
                                            break
                ...
                logger.info(f"第 {attempt + 1} 次查找，找到 {elements_found} 个元素，当前用户数: {len(user_nicknames)}")

                # 如果已经找到足够用户，停止滚动
                if len(user_nicknames) >= max_users:
                    logger.info(f"已找到足够用户，停止滚动")
                    break
                
                # 如果这次没有找到新用户，检查是否需要继续滚动
                current_user_count = len(user_nicknames)
                if current_user_count == previous_user_count:
                    no_new_user_count += 1
                    if no_new_user_count >= 3:  # 如果连续3次没有找到新用户
                        logger.info(f"连续 {no_new_user_count} 次未发现新用户，停止滚动")
                        break
                else:
                    no_new_user_count = 0  # 重置计数器
                ...
                if 'currentY' in scroll_result and 'newY' in scroll_result:
                    if scroll_result['currentY'] == scroll_result['newY'] and attempt > 0:
                        logger.info("已滚动到底部，停止搜索")
                        break

                # 等待滚动后页面稳定
                time.sleep(2.5)  # 增加等待时间，确保新内容加载
